...
AND bs.base_set_type = 'RS_MarketBasket'
--AND  set_sl.config_id = 551
order by 1),
fv as (  
   	
	select te.label as FValu  ,'Market Basket Scope' as Fld, ev.validation_set_config_id as config_id
   from
  ...
  where
   ...
   and tg.label = 'Market Basket Scope'   
   ) 
select
distinct
