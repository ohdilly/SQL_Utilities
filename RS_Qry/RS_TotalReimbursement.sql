...
AND bs.base_set_type = 'RS_TotalReimbursement'
--AND  set_sl.config_id = 551
order by 1),
fv as (  
   	
	select ev.value as FValu  ,'Pricing Basis' as Fld, ev.validation_set_config_id as config_id
   from
  ...
  where
   ...
   and tg.label = 'Pricing Basis' union
   
   	select ev.value ,'Percentage', ev.validation_set_config_id 
   from
   ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
