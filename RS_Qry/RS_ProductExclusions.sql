...
and bs.status  in ( 'ACTIVE', 'READY', 'DRAFT')
AND bs.base_set_type = 'RS_ProductExclusions'
--AND  set_sl.config_id = 551
order by 1),
fv as (  
   	
	select nvl(ev.value, 'NONE') as FValu  ,'Excluded Prod ID' as Fld, ev.validation_set_config_id as config_id
   from
  ...
  where
   ...
   AND tg.label = 'Exclude Product List'   
