...
AND bs.base_set_type = 'RS_reimbursement_amount'
...
fv as (  
  ...
  where
   ...
   AND ev.tab_element_id = 402   union
  ...
  where
   ev.tab_element_id = te.tab_element_id
   And te.tab_group_id = tg.tab_group_id
   AND ev.tab_element_id = 403   union
  ...
  where
   ev.tab_element_id = te.tab_element_id
   And te.tab_group_id = tg.tab_group_id
   AND ev.tab_element_id = 404   union
 ...
 select ev.value as FValu  ,tg.label as Fld, ev.validation_set_config_id as config_id
  ...
  where
   ev.tab_element_id = te.tab_element_id
   And te.tab_group_id = tg.tab_group_id
   AND ev.tab_element_id = 405   union
   
    select decode(ev.value, 1, 'Default',ev.value) as FValu  ,tg.label as Fld, ev.validation_set_config_id as config_id
   from
   ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
