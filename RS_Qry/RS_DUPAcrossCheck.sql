...
AND bs.base_set_type = 'RS_DUPAcrossCheck'
--AND  set_sl.config_id = 551
order by 1),
fv as (  
  select(listagg( DISTINCT ev.value, ',') )as FValu , 'Key Data Elements' as Fld, ev.validation_set_config_id as config_id
  from 
  ...
  where
   ev.tab_element_id = te.tab_element_id 
   AND ev.value ='true'
   AND te.tab_element_id in (12,13,14)
    group by ev.validation_set_config_id union  
	
	      select 
	decode(te.label, 'Summarized', 'Publised', 'Not Summarized' , 'Not Published', te.label ),'Line Publish Status', ev.validation_set_config_id 
   from  
  ivd_element_value ev,
  ivd_tab_element te
  where
   ev.tab_element_id = te.tab_element_id 
   AND ev.value is not null
   AND te.tab_element_id in (15,49,50)
   union 
  ...
  where
   ...
   and tg.label = 'Date Range' 
    group by ev.validation_set_config_id union
	  ...
	  select  listagg(te.label, ',') , 'Duplicate check processed', ev.validation_set_config_id 
  from 
  ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
  where
   ...
   And te.tab_group_id = tg.tab_group_id
   and ev.value is not null
   and tg.label = 'Duplicate Check Processed' 
   group by ev.validation_set_config_id  union 
  ...
  where
   ...
   and tg.label = 'Debit/Credit Pairs' union 
       ...
       select  ev.value, 'Mark Duplicate', ev.validation_set_config_id 
  from 
  ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
  where
   ...
   And te.tab_group_id = tg.tab_group_id
   and ev.value is not null
   and tg.label = 'Mark Duplicate' union
