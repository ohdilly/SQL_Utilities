...
AND bs.base_set_type = 'RS_DUPWithinCheck'
...
fv as (  
  ...
  where
   ...
   And te.tab_group_id = tg.tab_group_id
   and ev.value is not null
   and tg.label = 'MBC Duplicate Line Consistency' union
   
   --mark duplicate , w2astage dosage, variable quatity
     select  te.label, 'Mark Duplicate', ev.validation_set_config_id 
  from 
  ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
  ...
  where
   ...
   and tg.label = 'Wastage dosage handling' union
           ...
           select  te.label, 'Variable Quantity Handling', ev.validation_set_config_id 
  from 
  ivd_element_value ev,
  ivd_tab_element te,
  ivd_tab_group tg
  where
   ...
   And te.tab_group_id = tg.tab_group_id
   and ev.value is not null
   and tg.label = 'Variable Quantity Handling' 
