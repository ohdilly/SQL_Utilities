...
LOOP
  ...
  delete from validata.az_vd_gt_key_conv_file_ids;
  INSERT INTO validata.az_vd_gt_key_conv_file_ids (file_id,source_group_key)
    SELECT FILE_ID, source_group_key  
    FROM VALIDATA.IVD_TRANSACTION_FILE f WHERE 
    status <> 11 and
    START_DATE >= '01-JAN-2000' 
    and not exists
      (select 'x' from validata.AZ_ITF_KEYS k where  k.file_id = f.file_id) 
    and rownum < 100;
    ...
    INSERT /*+ parallel(orig,8)*/INTO AZ_IVD_TR_ORIG orig
      ...
      SELECT /*+ parallel (tr, 8)*/
       ...
       WHERE (tr.FILE_ID, tr.source_group_key) IN
             (select file_id, source_group_key from validata.az_vd_gt_key_conv_file_ids)
         and status <> 4;
  EXCEPTION
    WHEN OTHERS THEN
    ...
    UPDATE VALIDATA.ivd_reversal r
       SET r.contract_key =
           (SELECT AZ.CONTRACT_KEY
              FROM validata.AZ_MN_CONT AZ
             where AZ.CONT_NUM = R.contract_key)
     WHERE R.FILE_ID in (select file_id from validata.az_vd_gt_key_conv_file_ids)
  ...
  BEGIN
    UPDATE VALIDATA.ivd_reversal r
       SET r.source_key =
           ...
           WHERE FILE_ID in (select file_id from validata.az_vd_gt_key_conv_file_ids) 
...
--Update IVD_TRANSACTION_FILE : contract_key
  BEGIN
    ...
    UPDATE VALIDATA.ivd_transaction_file f
     ...
     WHERE F.FILE_ID in (select file_id from validata.az_vd_gt_key_conv_file_ids)
       AND EXISTS (SELECT AZ.CONTRACT_KEY
              FROM validata.AZ_MN_CONT AZ
             where AZ.CONT_NUM = F.contract_key);
  EXCEPTION
...
-------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    UPDATE /*+ parallel(r,16) */ VALIDATA.ivd_transaction_records r
       SET r.contract_key =
         (SELECT AZ.CONTRACT_KEY FROM validata.AZ_MN_CONT AZ where AZ.CONT_NUM = R.contract_key),
         r.product_key =
           (SELECT prod_rc.product_key 
              FROM validata.AZ_MN_PROD prod_rc
             WHERE prod_rc.product_num = r.product_key),
         r.PLAN_KEY =
           (SELECT mn_cust 
              FROM validata.AZ_MN_PLAN AZ
             WHERE AZ.mbr_bunit_num = r.plan_key)
       WHERE (r.FILE_ID, r.source_group_key) in ( 
      SELECT file_id, source_group_key
        FROM validata.az_vd_gt_key_conv_file_ids f ) 
         AND r.status <> 4;
  EXCEPTION
    WHEN OTHERS THEN
